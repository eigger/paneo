...
| C1 | 서드파티 개방 | **단계적** (내장→검증→샌드박스 개방) | 개방은 M6 |
| C2 | MVP 위젯 | **7종** (시계·날씨·캘린더·사진·RSS·텍스트·iframe) | |
...
Device        기기. { id, name, token,                            // ✅ 구현됨 (M0/M1)
                      ...
                      powerSchedule?, agentPresent: bool }        // ⏳ M4 예정 (§9 화면 전원)
...
- **주의**: MVP에서 임의 원격 코드 실행을 바로 허용하지 말 것. 내장 위젯 → 검증된 위젯 레지스트리 → 완전 개방 순으로 단계적 개방.
...
- ✅ **M0 — 뼈대**: 서버(발행 허브+SQLite) + kiosk 페이지 + "적용" 시 레이아웃 반영. "편집기에서 위젯 배치 후 적용을 누르면 Pi 화면이 바뀐다"를 최단으로 증명. (2026-07-03 완료, 실 클릭 기준 검증)
- ✅ **M1 — 편집기**: 드래그&드롭 그리드, 초안/발행, **SQLite 전환(`node:sqlite`)**, 설정 폼(text/checkbox/number/textarea), **i18n 골격(편집기 ko/en + 기기 로케일, §4.4)**, **데이터 프록시(§4.2, weather/ical/rss)**, **내장 위젯 8종**(시계·날짜·텍스트·날씨·캘린더·RSS·iframe·사진 슬라이드쇼 — MVP 7종 §7.4 조기 달성). (2026-07-03 완료, 실 클릭 기준 검증)
- **M2 — 기기 관리**: 기기 등록/페어링, 그룹, 레이아웃 배정, 성능 프로파일, 원격 reload/identify.
- **M3 — 플러그인 규격 확정**: 위젯을 정식 매니페스트(§7.1)로 리팩터 + 동적 로딩 준비, 오프라인 캐시. (위젯 자체는 M1에서 이미 구현됨)
- **M4 — 컴패니언 에이전트**: 화면 전원 스케줄/원격 제어(§9), 건강 상태 보고.
- **M5 — 확장 연동**: Home Assistant 데이터소스(§8.3), 이미지 액자 모드(§8.2).
- **M6 — RTSP & 서드파티**: 카메라 게이트웨이 위젯(§8.1), 위젯 동적 로딩 + 샌드박스 + 권한 모델.
...
5. **RTSP 부하**: 영상 트랜스코딩은 서버/게이트웨이 부하 큼(§8.1). 저성능 호스트에선 스냅샷 폴백 필수.
...
---
### 다음 액션
- [ ] 타깃 Pi 모델 확정 (성능 예산의 출발점 — 고성능/저성능 tier 경계)
- [ ] M0 프로토타입 착수 여부 결정
